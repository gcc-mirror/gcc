...
# endif
  ...
  if (__builtin_add_overflow(sz, al - 1, &sz)) [[__unlikely__]]
    return nullptr;
  sz &= ~(al - 1);
...
aligned_alloc (std::size_t al, std::size_t sz)
{
  ...
  std::size_t check;
  if (__builtin_add_overflow(sz, al + 2 * sizeof(void*), &check)) [[__unlikely__]]
    return nullptr;
...
aligned_alloc (std::size_t al, std::size_t sz)
{
  ...
  if (al < sizeof(void*))
    al = sizeof(void*);
  if (__builtin_add_overflow(sz, al, &sz)) [[__unlikely__]]
    return nullptr;
  void* const malloc_ptr = malloc(sz);
  if (!malloc_ptr) [[__unlikely__]]
    return nullptr;
