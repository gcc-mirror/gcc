...
{
  const int i = 42;
  auto o = std::pmr::make_optional(i);
  static_assert( std::is_same<decltype(o), std::pmr::optional<int>>(), "" );
  VERIFY( o && *o == 42 );
  VERIFY( &*o != &i );
  auto o2 = std::pmr::make_optional<std::tuple<int, int>>(1, 2);
  static_assert( std::is_same<decltype(o2),
		 std::pmr::optional<std::tuple<int, int>>>(), "" );
  VERIFY( o2 && std::get<0>(*o2) == 1 && std::get<1>(*o2) == 2);
  auto o3 = std::pmr::make_optional<std::vector<int>>({42, 666});
  static_assert( std::is_same<decltype(o3),
		 std::pmr::optional<std::vector<int>>>(), "" );
  VERIFY(o3 && (*o3)[0] == 42 && (*o3)[1] == 666);
  auto o4 = std::pmr::make_optional<combined>({42, 666});
  static_assert( std::is_same<decltype(o4),
		 std::pmr::optional<combined>>(), "" );
  VERIFY(o4 && (o4->v)[0] == 42 && (o4->v)[1] == 666
	 && std::get<0>(o4->t) == 0 && std::get<1>(o4->t) == 0 );
  auto o5 = std::pmr::make_optional<combined>({1, 2}, 3, 4);
