...
static_assert(hashable<std::pmr::optional<int>>());
static_assert(hashable<std::pmr::optional<const int>>());

int main()
{
  int x = 42;
  std::pmr::optional<int> x2 = 42;
  VERIFY(std::hash<int>()(x) == std::hash<std::pmr::optional<int>>()(x2));

  // PR libstdc++/82262
  std::pmr::optional<const int> x3 = x2;
  VERIFY(std::hash<int>()(x) == std::hash<std::pmr::optional<const int>>()(x3));
}
