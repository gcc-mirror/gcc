...
{
  {
    static_assert( std::is_trivially_destructible<std::pmr::optional<trivially_destructible>>(), "" );
  }

  {
    using T = no_default_constructor;
    using O = std::pmr::optional<T>;
...

  {
     ...
     * time), this does not affect the std::pmr::optional implementation however. So the assertion
...

  {
     ...
     */
    using A = std::pmr::optional<int&>;
    using B = std::pmr::optional<int&&>;
    using C1 = std::pmr::optional<std::in_place_t>;
    using C2 = std::pmr::optional<std::in_place_t const>;
    using C3 = std::pmr::optional<std::in_place_t volatile>;
    using C4 = std::pmr::optional<std::in_place_t const volatile>;
    using D1 = std::pmr::optional<std::nullopt_t>;
    using D2 = std::pmr::optional<std::nullopt_t const>;
    using D3 = std::pmr::optional<std::nullopt_t volatile>;
    using D4 = std::pmr::optional<std::nullopt_t const volatile>;

    using X = std::tuple<A, B, C1, C2, C3, C4, D1, D2, D3, D4>;
  }

  {
    std::pmr::optional<const int> o { 42 };
  ...
  {
    constexpr std::pmr::optional<const int> o { 33 };
    static_assert( std::is_same<decltype(o)::value_type, const int>(), "" );
    static_assert( o, "" );
    static_assert( *o == 33, "" );
  }
