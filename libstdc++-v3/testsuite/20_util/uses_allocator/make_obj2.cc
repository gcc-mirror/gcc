...
struct B
{
	...
	B(std::allocator_arg_t, const my_alloc& a)
	: nargs(0), alloc_id(a.id) { }
	B(std::allocator_arg_t, const my_alloc& a, float&)
	: nargs(1), alloc_id(a.id) { }
	B(std::allocator_arg_t, const my_alloc& a, int, void*)
	: nargs(2), alloc_id(a.id) { }
	B(std::allocator_arg_t, const my_alloc& a, B&& b)
	: nargs(b.nargs), alloc_id(a.id) { }
...
namespace std {
template<>
struct __domain_allocator_traits<B, test_allocator> : std::true_type {
    static my_alloc  convert(test_allocator _a) { return my_alloc{_a.get_personality()}; }
};
}
struct C
{
	static my_alloc  domain_alloc_convert(test_allocator _a)
	{ return my_alloc{_a.get_personality()}; }
	...
	C(const my_alloc& a)
	: nargs(0), alloc_id(a.id) { }
	C(float&, const my_alloc& a)
	: nargs(1), alloc_id(a.id) { }
	C(int, void*, const my_alloc& a)
	: nargs(2), alloc_id(a.id) { }
	C(C&& c, const my_alloc& a)
	: nargs(c.nargs), alloc_id(a.id) { }
...
test04()
{
  struct D
  {
    ...
    // These should not be used:
    D(std::allocator_arg_t, const my_alloc&);
    D(std::allocator_arg_t, const my_alloc&, int);
  ...
  struct E
  {
    ...
    E(std::allocator_arg_t, const my_alloc&) { }
    E(std::allocator_arg_t, int, const my_alloc&) { }
