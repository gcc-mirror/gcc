...
/* Prepare for tokenizing a C++2a module/import directive.  This
   behaves like a deferred pragma.  It is the responsibility of the
   token consumer to deal with actual importing, including getting
   header-unit macros.  */

bool
_cpp_setup_module_directive (cpp_reader *pfile, cpp_token *first_token,
			     cpp_token *keyword_token, int importedness)
{
  /* Our caller will have done some peeking and set up directive
     state.  */
  ...
  if (pfile->state.pragma_allow_expansion)
    {
      /* We never turned off macro expansion.  */
      gcc_checking_assert (!pfile->state.prevent_expansion);

      if (!importedness && linemap_included_from
	  (LINEMAPS_LAST_ORDINARY_MAP (pfile->line_table)))
	cpp_error_with_line (pfile, CPP_DL_ERROR, keyword_token->src_loc, 0,
			     "module control-line cannot be in included file");

      /* The first one or two tokens cannot be macro names.  */
      for (int ix = 1 + (keyword_token != first_token); ix--;)
	{
	  cpp_token *tok = ix ? keyword_token : first_token;
      ...
      keyword_token->val.node.node
	= (importedness
	   ? pfile->spec_nodes.n__import : pfile->spec_nodes.n__module);
      if (keyword_token != first_token)
	first_token->val.node.node = pfile->spec_nodes.n__export;
    }

  // FIXME: We have to arrange to re-tokenize the import argument as
  // an include name, if peek is a HEADER_NAME or <.  At that point we
  // do include lookup.

  pfile->directive_result.src_loc = first_token->src_loc;
  pfile->directive_result.type = CPP_PRAGMA;
  pfile->directive_result.flags = first_token->flags;
  pfile->directive_result.val.pragma = 0;

  pfile->state.in_deferred_pragma = true;

  /* Our caller will restore the remaining directive state.  */
  return true;
}
