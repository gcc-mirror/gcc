...
{
  CAN_REMOVE,		/* This node can be removed, or is still to be analysed.  */
  ...
  IN_BOUNDARY,		/* This node is in boundary, therefore being in other
			   partition or is a external symbol, and its body can be
			   released.  */
...
handle_node_in_boundary (symtab_node *node, bool keep_body)
{
  if (cgraph_node *cnode = dyn_cast <cgraph_node *> (node))
	...
	{
	  /* Marked to be inlined into a node not in current partition?
...
compute_boundary (ltrans_partition partition)
{
  ...
  /* Then look for nodes that was marked to be inlined to. If it is marked to be
     inlined into a node that is in current partition, then mark its body to
