	  ...
	  if (candidates[i].alternative == fndecl)
	  ...
	  else
	    {
		 ...
		 and just replace the function.  */
	      ...
	      if (omp_dispatch_p)
		thiscall
		  = modify_call_for_omp_dispatch (thiscall,
						  gimplify_omp_ctxp->clauses,
						  want_value, pointerize);
	      if (!tail)
		tail = thiscall;
	      else
...
gimplify_call_expr (tree *expr_p, gimple_seq *pre_p, fallback_t fallback)
{
  ...
  /* Handle "declare variant" substitution and arglist processing.  */
  if (flag_openmp
      && fndecl
      && cfun
			   ...
			   DECL_ATTRIBUTES (fndecl)))
    {
      tree orig = *expr_p;
      enum gimplify_status ret
	= gimplify_variant_call_expr (expr_p, pre_p, fallback, omp_dispatch_p);
