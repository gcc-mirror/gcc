...
class Keywords {
  public:
    ...
    Keyword keyword_to_code(const char* keyword, size_t len) const;

    // Return the string for a keyword.
    const char* keyword_to_string(Keyword) const;
...
const Keywords::Mapping
    Keywords::mapping_[] = {
        ...
        { "switch", KEYWORD_SUPER },
        { "trait", KEYWORD_TRAIT },
        { "true", KEYWORD_TRUE },
        { "type", KEYWORD_TYPE },
        { "typeof", KEYWORD_TYPEOF },
        { "unsafe", KEYWORD_UNSAFE },
        { "unsized", KEYWORD_UNSIZED },
        { "use", KEYWORD_USE },
...
const int Keywords::count_ = sizeof(Keywords::mapping_) / sizeof(Keywords::mapping_[0]);
...
static int keyword_compare(const void* keyv, const void* mapv) {
    const Keywords_search_key* key = static_cast<const Keywords_search_key*>(keyv);
...
Keyword Keywords::keyword_to_code(const char* keyword, size_t len) const {
...
const char* Keywords::keyword_to_string(Keyword code) const {
...
void Token::clear() {
    if (this->classification_ == TOKEN_INTEGER || this->classification_ == TOKEN_CHARACTER)
        mpz_clear(this->u_.integer_value);
    else if (this->classification_ == TOKEN_FLOAT || this->classification_ == TOKEN_IMAGINARY)
        mpfr_clear(this->u_.float_value);
}

// Construct a token.

Token::Token(const Token& tok) : classification_(tok.classification_), location_(tok.location_) {
        ...
        case TOKEN_OPERATOR:
            ...
            switch (this->u_.op) {
                case OPERATOR_INVALID:
                    fprintf(file, "invalid");
                    break;
                case OPERATOR_DOUBLE_PIPE:
                    fprintf(file, "||");
                    break;
                case OPERATOR_DOUBLE_AMP:
                    fprintf(file, "&&");
                    break;
                case OPERATOR_DOUBLE_EQ:
                    fprintf(file, "==");
                    break;
                case OPERATOR_EXCLAM_EQ:
                    fprintf(file, "!=");
                    break;
                case OPERATOR_LT:
                    fprintf(file, "<");
                ...
                case OPERATOR_EXCLAM:
                    fprintf(file, "!");
                    break;
                case OPERATOR_LEFT_ARROW:
                    fprintf(file, "<-");
                    break;
                case OPERATOR_RIGHT_ARROW:
                    fprintf(file, "->");
                    break;
                case OPERATOR_EQ:
                    fprintf(file, "=");
                ...
                case OPERATOR_MINUS_EQ:
                    fprintf(file, "-=");
                    break;
                case OPERATOR_PIPE_EQ:
                    fprintf(file, "|=");
                    break;
                case OPERATOR_CARET_EQ:
                    fprintf(file, "^=");
                    break;
                ...
                case OPERATOR_HASH:
                    fprintf(file, "#");
                    break;
                case OPERATOR_DOUBLE_QUOTE:
                    fprintf(file, "\"");
                    break;
                case OPERATOR_SINGLE_QUOTE:
                    fprintf(file, "'");
                    break;
                ...
                case OPERATOR_BACKSLASH:
                    fprintf(file, "\\");
                    break;
                case OPERATOR_BLOCK_COMMENT:
                    fprintf(file, "/*");
                    break;
                case OPERATOR_LINE_COMMENT:
                    fprintf(file, "//");
                    break;
                case OPERATOR_QMARK:
                    fprintf(file, "?");
        ...
        while (p < pend) {
            ...
            switch (cc) {
                ...
                default: {
                    ...
                    bool issued_error;
                    this->lineoff_ = p - this->linebuf_;
                    const char* pnext = this->advance_one_utf8_char(p, &ci, &issued_error);
...
int Lex::fetch_char(const char* p, unsigned int* value) {
    unsigned char c = *p;
    if (c <= 0x7f) {
        *value = c;
        return 1;
        ...
        *value = (((c & 0xf) << 12) + ((p[1] & 0x3f) << 6) + (p[2] & 0x3f));
        if (*value <= 0x7ff) {
            *value = 0xfffd;
            return 0;
        }
        ...
        *value = (((c & 0x7) << 18) + ((p[1] & 0x3f) << 12) + ((p[2] & 0x3f) << 6) + (p[3] & 0x3f));
        if (*value <= 0xffff) {
            *value = 0xfffd;
            return 0;
        }
...
const char*
Lex::advance_one_utf8_char(const char* p, unsigned int* value, bool* issued_error) {
    *issued_error = false;
...
static const unsigned char hex_value_lookup_table[256] =
  {
      ...
      NHV,
      NHV,
      NHV, // NUL SOH STX ETX EOT ENQ ACK BEL
      NHV,
      NHV,
      NHV,
      NHV,
      ...
      NHV,
      NHV,
      NHV, // DLE DC1 DC2 DC3 DC4 NAK SYN ETB
      NHV,
      NHV,
      NHV,
      NHV,
      ...
      NHV,
      NHV,
      NHV, // CAN  EM SUB ESC  FS  GS  RS  US
      NHV,
      NHV,
      NHV,
      NHV,
...
bool Lex::could_be_exponent(const char* p, const char* pend) {
    if (*p != 'e' && *p != 'E')
        return false;
    ++p;
    if (p >= pend)
...
Token Lex::gather_number() {
    ...
    if (*p == '0') {
        int base;
        if ((p[1] == 'x' || p[1] == 'X') && Lex::is_hex_digit(p[2])) {
            base = 16;
            p += 2;
            pnum = p;
            while (p < pend) {
...
const char*
Lex::advance_one_char(const char* p, bool is_single_quote, unsigned int* value, bool* is_character) {
    *value = 0;
    *is_character = true;
    if (*p != '\\') {
        bool issued_error;
        ...
        switch (*p) {
            ...
            case '7':
                *is_character = false;
                if (p[1] >= '0' && p[1] <= '7' && p[2] >= '0' && p[2] <= '7') {
                    *value = ((Lex::octal_value(p[0]) << 6) + (Lex::octal_value(p[1]) << 3) + Lex::octal_value(p[2]));
                    if (*value > 255) {
            ...
            case 'u':
                if (Lex::is_hex_digit(p[1]) && Lex::is_hex_digit(p[2]) && Lex::is_hex_digit(p[3]) && Lex::is_hex_digit(p[4])) {
                    *value = ((Lex::hex_val(p[1]) << 12) + (Lex::hex_val(p[2]) << 8) + (Lex::hex_val(p[3]) << 4) + Lex::hex_val(p[4]));
            ...
            case 'U':
                if (Lex::is_hex_digit(p[1]) && Lex::is_hex_digit(p[2]) && Lex::is_hex_digit(p[3]) && Lex::is_hex_digit(p[4]) && Lex::is_hex_digit(p[5]) && Lex::is_hex_digit(p[6]) && Lex::is_hex_digit(p[7]) && Lex::is_hex_digit(p[8])) {
                    *value = ((Lex::hex_val(p[1]) << 28) + (Lex::hex_val(p[2]) << 24) + (Lex::hex_val(p[3]) << 20) + (Lex::hex_val(p[4]) << 16) + (Lex::hex_val(p[5]) << 12) + (Lex::hex_val(p[6]) << 8) + (Lex::hex_val(p[7]) << 4) + Lex::hex_val(p[8]));
                    if (*value > 0x10ffff || (*value >= 0xd800 && *value < 0xe000)) {
                        go_error_at(this->location(),
                                    "invalid unicode code point 0x%x",
                                    *value);
                        // Use the replacement character.
...
void Lex::append_char(unsigned int v, bool is_character, std::string* str, Location location) {
    char buf[4];
    size_t len;
    if (v <= 0x7f || !is_character) {
        ...
        if (v > 0x10ffff) {
            go_warning_at(location, 0, "unicode code point 0x%x out of range in string", v);
            // Turn it into the "replacement character".
            v = 0xfffd;
        }
        if (v >= 0xd800 && v < 0xe000) {
            go_warning_at(location, 0, "unicode code point 0x%x is invalid surrogate pair", v);
            v = 0xfffd;
        }
        if (v <= 0xffff) {
            buf[0] = 0xe0 + (v >> 12);
...
bool Lex::skip_c_comment(bool* found_newline) {
    while (true) {
        if (!this->require_line()) {
...
void Lex::skip_cpp_comment() {
    ...
    while (pend > p && (pend[-1] == ' ' || pend[-1] == '\t' || pend[-1] == '\r' || pend[-1] == '\n'))
        --pend;
    ...
    if (pend - p > 5 && memcmp(p, "line ", 5) == 0) {
        ...
        if (pcolon != NULL && pcolon[1] >= '0' && pcolon[1] <= '9') {
            char* plend;
            long lineno = strtol(pcolon + 1, &plend, 10);
            if (plend > pcolon + 1 && (plend == pend || *plend < '0' || *plend > '9') && lineno > 0 && lineno < 0x7fffffff) {
...
bool Lex::is_in_unicode_range(unsigned int c, const Unicode_range* ranges, size_t range_size) {
    if (c < 0x100) {
...
bool Lex::is_unicode_space(unsigned int c) {
    return Lex::is_in_unicode_range(c, unicode_space, ARRAY_SIZE(unicode_space));
}

// Return whether C is a Unicode digit--a Unicode code point
// classified as "Digit".

bool Lex::is_unicode_digit(unsigned int c) {
    return Lex::is_in_unicode_range(c, unicode_digits, ARRAY_SIZE(unicode_digits));
}

// Return whether C is a Unicode letter--a Unicode code point
// classified as "Letter".

bool Lex::is_unicode_letter(unsigned int c) {
    return Lex::is_in_unicode_range(c, unicode_letters, ARRAY_SIZE(unicode_letters));
}

// Return whether C is a Unicode uppercase letter.  a Unicode code
// point classified as "Letter, uppercase".

bool Lex::is_unicode_uppercase(unsigned int c) {
    return Lex::is_in_unicode_range(c, unicode_uppercase_letters, ARRAY_SIZE(unicode_uppercase_letters));
}

// Return whether the identifier NAME should be exported.  NAME is a
// mangled name which includes only ASCII characters.

bool Lex::is_exported_mangled_name(const std::string& name) {
...
bool Lex::is_exported_name(const std::string& name) {
    unsigned int uchar;
...
bool Lex::is_invalid_identifier(const std::string& name) {
