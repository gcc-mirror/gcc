...
/* Return whether strings S1 and S2 are both NULL or both the same string.  */
/* static bool strings_same(const char* s1, const char* s2) {
    return s1 == s2 || (s1 != NULL && s2 != NULL && strcmp(s1, s2) == 0);
}*/

/* Return whether decoded option structures OPT1 and OPT2 are the same.  */
...
/* Append another argument to the list being built.  As long as it is identical to the corresponding
 * arg in the original list, just increment the new arg count.  Otherwise allocate a new list, etc. */
...
/* Append an option described by OPT_INDEX, ARG and VALUE to the list being built.  */
...
/* Append a librust argument to the list being built. If FORCE_STATIC, ensure the library is linked
 * statically.  */
...
/* Called before linking - can be used to do some extra steps and abort if they fail. Returns 0 on
...
/* Number of extra output files that lang_specific_pre_link may generate. Used to add extra outfiles
 * to linking step. */
