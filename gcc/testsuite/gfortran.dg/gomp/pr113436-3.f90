...
!  use omp_lib
  ...
  !$omp target private(A) allocate(allocator(omp_high_bw_mem_alloc), align(16): A)
      do i = 1, 10
        A(i) = -i + 23
      end do
  !$omp end target
...
! { dg-final { scan-tree-dump "D\\\.\[0-9\]\+ = __builtin_GOMP_alloc \\\(16, 40, 4\\\);" "omplower" { target int32 } } }
