...
! { dg-final { scan-tree-dump "omp target exit data map\\(delete:one \\\[len: .*\\\]\\) map\\(delete:.*\\)" "omplower" } }
! { dg-final { scan-tree-dump "omp target exit data map\\(release:two \\\[len: .*\\\]\\) map\\(release:.*\\)" "omplower" } }
