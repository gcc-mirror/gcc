...
module m3
 ...
 interface
  ...
  subroutine f0(x,y,z)
    ...
    !$omp&                   match ( construct = { dispatch } )
! { dg-warning "62: Expected positive argument index at .1. \\\[-Wopenmp\\\]" "" { target *-*-* } .-2 }
! { dg-warning "86: Upper argument index smaller than lower one at .1. \\\[-Wopenmp\\\]" "" { target *-*-* } .-3 }
  end subroutine
 end interface
end module m3
...
module m4
 ...
 interface
  ...
  subroutine f0(x,y,z)
    ...
    !$omp&                   match ( construct = { dispatch } )
! { dg-error "86: 'x' at .1. is specified more than once" "" { target *-*-* } .-4 }
! { dg-error "86: 'y' at .1. is specified more than once" "" { target *-*-* } .-5 }
! { dg-error "78: 'z' at .1. is specified more than once" "" { target *-*-* } .-4 }
  end subroutine
 end interface
end module m4
