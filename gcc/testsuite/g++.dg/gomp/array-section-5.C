...
// See 'array-section-6.C' for C++26 and higher
...
void f() {
 ...
 #pragma omp task affinity(c2[: :])
  ;
 // { dg-error "33: expected primary-expression before ':' token" "" { target *-*-* } .-2 }
 ...
 #pragma omp task affinity(c3[:1:])
  ;
 // { dg-error "33: expected '\\\]' before ':' token" "" { target *-*-* } .-2 }
 // { dg-error "34: expected an OpenMP clause before '\\\]' token" "" { target *-*-* } .-3 }
 ...
 #pragma omp task affinity(c5[ :1:])
  ;
 // { dg-error "34: expected '\\\]' before ':' token" "" { target *-*-* } .-2 }
 // { dg-error "35: expected an OpenMP clause before '\\\]' token" "" { target *-*-* } .-3 }
}
