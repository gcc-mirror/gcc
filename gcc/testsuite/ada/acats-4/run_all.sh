#!/bin/sh
# Run ACATS 4 with the GNU Ada compiler
...
find_main () {
  ls ${i}[a-z].adb  > ${i}.lst 2> /dev/null
  ls ${i}[0-9].adb >> ${i}.lst 2> /dev/null
...
if [ -n "$GCC_RUNTEST_PARALLELIZE_DIR" ]; then
  dir_support=$dir/../acats-4/support
