...
*/
long
signed_fetch_xor_hint (int x, int *p)
{
  return __arm_atomic_fetch_xor_with_hint (p, x, __ATOMIC_RELAXED, 1);
}


/*
** unsigned_fetch_xor_hint:
** ...
**	shuh	ph
**	ldeor	(w[0-9]+), \1, \[x[0-9]+\]
**	uxtw	x[0-9]+, \1
...
*/
long
unsigned_fetch_xor_hint (unsigned int x, unsigned int *p)
{
  return __arm_atomic_fetch_xor_with_hint (p, x, __ATOMIC_RELAXED, 1);
}