...
*/
long
signed_fetch_or_hint (int x, int *p)
{
  return __arm_atomic_fetch_or_with_hint (p, x, __ATOMIC_RELAXED, 1);
}


/*
** unsigned_fetch_or_hint:
** ...
**	shuh	ph
**	ldset	(w[0-9]+), \1, \[x[0-9]+\]
**	uxtw	x[0-9]+, \1
...
*/
long
unsigned_fetch_or_hint (unsigned int x, unsigned int *p)
{
  return __arm_atomic_fetch_or_with_hint (p, x, __ATOMIC_RELAXED, 1);
}