...
impl<K, V> BoxedNode<K, V> {
    fn from_leaf(node: Box<LeafNode<K, V>>) -> Self {
        BoxedNode {
            ptr: Unique::from(Box::leak(node)),
        }
    }

    fn from_internal(node: Box<InternalNode<K, V>>) -> Self {
        BoxedNode {
            ptr: Unique::from(Box::leak(node)).cast(),
        }
    }
    ...
    pub fn push(&mut self, key: K, val: V, edge: Root<K, V>) {
        ...
        *len += 1;
        unsafe {
            ...
            self.reborrow_mut().into_val_area_mut_at(idx).write(val);
            self.reborrow_mut()
                .into_edge_area_mut_at(idx + 1)
                .write(edge.into_boxed_node());
    ...
    fn push_front(&mut self, key: K, val: V, edge: Root<K, V>) {
        ...
        unsafe {
            *self.reborrow_mut().into_len_mut() += 1;
            ...
            slice_insert(self.reborrow_mut().into_val_area_slice(), 0, val);
            slice_insert(
                self.reborrow_mut().into_edge_area_slice(),
                0,
                edge.into_boxed_node(),
            );
        }
    ...
    fn insert_fit(&mut self, key: K, val: V) -> *mut V {
        ...
        unsafe {
            *self.node.reborrow_mut().into_len_mut() += 1;
            slice_insert(
                self.node.reborrow_mut().into_key_area_slice(),
                self.idx,
                key,
            );
            slice_insert(
                self.node.reborrow_mut().into_val_area_slice(),
                self.idx,
                val,
            );
    ...
    fn insert_fit(&mut self, key: K, val: V, edge: Root<K, V>) {
        ...
        unsafe {
            *self.node.reborrow_mut().into_len_mut() += 1;
            ...
            slice_insert(
                self.node.reborrow_mut().into_edge_area_slice(),
                self.idx + 1,
                boxed_node,
            );

            self.node
                .correct_childrens_parent_links((self.idx + 1)..=self.node.len());
        }
    }
...
impl<'a, K: 'a, V: 'a, NodeType> Handle<NodeRef<marker::Immut<'a>, K, V, NodeType>, marker::KV> {
    pub fn into_kv(self) -> (&'a K, &'a V) {
        (unsafe { self.node.key_at(self.idx) }, unsafe {
            self.node.val_at(self.idx)
        })
    }
}
    ...
    pub fn split(
        ...
        unsafe {
            ...
            right
                ...
                .correct_childrens_parent_links(0..=new_len);
    ...
    pub fn merge(
        ...
        unsafe {
            *left_node.reborrow_mut().into_len_mut() += right_len as u16 + 1;
            ...
            slice_remove(
                &mut self.node.reborrow_mut().into_edge_area_slice(),
                self.idx + 1,
            );
            let self_len = self.node.len();
            self.node
                .correct_childrens_parent_links(self.idx + 1..self_len);
            *self.node.reborrow_mut().into_len_mut() -= 1;
...
//gccrs-commented-out because of issue #4687
