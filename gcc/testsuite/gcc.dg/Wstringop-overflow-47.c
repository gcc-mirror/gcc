...
/* The IL below that's visible to the warning changes from one target to
 ...
 extern char warn_a64[64];   // { dg-message "at offset (1|128) into destination object 'warn_a64' of size (63|64)" "pr97027 note" { xfail { ! { aarch64-*-* || { arm*-*-* && { arm_neon || { arm_mve || arm_thumb1 } } } } } } }

  void *p = warn_a64 + 1;
  I16_64 *q = (I16_64*)p;
  *q = (I16_64){ i };         // { dg-warning "writing (1 byte|64 bytes) into a region of size (0|63)" "pr97027" { xfail { ! { aarch64-*-* || { arm*-*-* && { arm_neon || { arm_mve || arm_thumb1 } } } } } } }
