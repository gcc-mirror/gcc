...
/* Empty loop bodies should still suppress coverage for the for (;;) and compile
   fine.  */
int
empty_body_for_loop ()
{
...
#pragma GCC suppress_coverage end
  ...
  // declarations with no init has no, and suppress_coverage is a no-op
#pragma GCC suppress_coverage begin
  int c;				/* count(-) */
#pragma GCC suppress_coverage end
...
{
  unsigned int ret = 0;
  /* With the pragma we can suppress condition, but still observe the then-else.  */
#pragma GCC suppress_coverage begin
  if (true_var)		/* count(#) */
#pragma GCC suppress_coverage end
    {
...
test_switch (int i, int j)
{
  ...
  /* We can suppress individual statements and breaks in the switch.  */
  switch (i)				/* count(5) */
    {
    case 1:
#pragma GCC suppress_coverage begin
...
#pragma GCC suppress_coverage end
  ...
  /* We can suppress the whole switch.  */
#pragma GCC suppress_coverage begin
  switch (i)
    {
    case 1:
...
test_switch2 (int i, int j)
{
  ...
  switch (i)				/* count(1) */
	...
	return do_something (4);	/* count(#####) */
      ...
      break;				/* count(#####) */
      /* We can suppress the label itself with the pragma.  */
#pragma GCC suppress_coverage begin
    default:				/* count(#) */
...
#pragma GCC suppress_coverage end
  ...
  /* We can suppress multiple cases with a single pragma.  */
#pragma GCC suppress_coverage begin
  switch (i)				/* count(#) */
    {
    case 1:				/* count(#) */
...
/* Returns, guarded by both plain values and function calls.
   ...
   This test demonstrates a surprising behaviour; because there are no braces
   but a single return statement, the count is assigned to the pragma line and
   not the return statement in the body.  If we don't store the pragma location
   we won't even record the return; as ignored.  In a perfect world this would
   be fixed by anchoring the return to the return statement itself and not the
   if.  If we add braces or return a value (see return2) this works as
   expected.  */
void
